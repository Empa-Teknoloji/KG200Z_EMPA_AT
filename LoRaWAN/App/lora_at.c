...
void AT_event_receive(LmHandlerAppData_t *appData, LmHandlerRxParams_t *params) {
	...
	if ((params != NULL) && (params->RxSlot < RX_SLOT_NONE)) {
		AT_PRINTF("+EVT:RX_%s, PORT %d, DR %d, RSSI %d, SNR %d",
...
void AT_event_Beacon(LmHandlerBeaconParams_t *params) {
	...
	if (params != NULL) {
		...
		case LORAMAC_HANDLER_BEACON_RX: {
			AT_PRINTF(
					"+EVT:RX_BC, DR %d, RSSI %d, SNR %d, FQ %d, TIME %d, DESC %d, "
...
/* --------------- AT Timer  command --------------- */
static UTIL_TIMER_Object_t event1;
static UTIL_TIMER_Object_t event2;
static UTIL_TIMER_Object_t event3;
static UTIL_TIMER_Object_t event4;

uint8_t timer_flag_1 = 0;
uint8_t timer_flag_2 = 0;
uint8_t timer_flag_3 = 0;
uint8_t timer_flag_4 = 0;
